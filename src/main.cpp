...
uint256 hashGenesisBlock("0x218abb16cbe8df616cec3624ececb76b30a4ea3926cb8b4479cba1d07a45edc8");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mobilecash: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Mobilecash Signed Message:\n";
...
{
    // Mobilecash: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1 ) nSubsidy = 1052101444 * COIN;
        ...
        nSubsidy = 20.60602 * COIN;
    }

    else {
        nSubsidy = 20.0 * pow(1.01, nHeight/525960) * COIN;
    }
...
static const int64 nTargetTimespan = 1.4 * 24 * 60 * 60; // Mobilecash: less hours
static const int64 nTargetSpacing = 60; // Mobilecash: 1 minute
...
{
        return GetNextWorkRequired_V1(pindexLast, pblock); // no KGW
}
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mobilecash: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Mobilecash: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
		...
		BOOST_FOREACH(const CTransaction& tx, pblock->vtx){
			...
			for(unsigned int i = 0; i < vtxOut.size(); i++){
				...
				if(txOut.scriptPubKey[0] == OP_RETURN && txOut.scriptPubKey[1] == 0x21){
					...
					// generate Mobilecash address from ephem_pubkey, scan_secret and spend_secret
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'l';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0xa67560e238f002985bcac02bc7162cadb65ccc598329ab6b639715558b8cc7ee");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25/Oct/2015 Raz sierpem raz mlotem PeOwska holote";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040cd4625a49aab268303860ccd6d66e1228039d0f5f19ea057c188b9c855c35bfae8d96c4d884cd6e68f2afd78617cf4b9d429b2007994ca3a41cb1e67de1fbc5") << OP_CHECKSIG;
        ...
        block.nNonce = 531895;
        block.nTime = 1441144114;

        if (fTestNet)
        {

            block.nNonce = 0;
            block.nTime = 1438240888;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x21a29c89e87d5242151cf77e0249c2c08353ffbd36d714fde10e84a49e6b6845"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbd, 0xb5, 0xd8 }; // Mobilecash sequence
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    else // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MobilecashMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MobilecashMiner : ProcessBlock, block not accepted");

    return true;
}

void static MobilecashMiner(CWallet *pwallet)
{
    printf("MobilecashMiner started\n");

    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mobilecash-miner");
    ...
    try { loop {
        ...
        printf("Running MobilecashMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MobilecashMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MobilecashMiner, pwallet));
}
